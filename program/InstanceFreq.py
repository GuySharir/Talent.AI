...
import pandas as pd
from program.ReadData import read_local_json_employees, read_attr_types_data, read_nested_attr_types_data, read_freq_per_value_data, MIN_FREQ, NUMERIC_DEFAULT
...
def nested_to_vec_representation(representation_option: DistMethod, representation_option_set: DistMethod, attr_name: str, freq_val: dict, nested_val: dict, instance_freq_vec: list) -> list:
    ...
    if representation_option == DistMethod.fix_length_freq:
        ...
        if not nested_val:
            ...
            instance_freq_vec.extend(freq_values_representation)
            logger(f'freq_values_representation- {freq_values_representation}')
            ...
            if len(nested_val) < attr_length:
                ...
                instance_freq_vec.extend(freq_values_representation)
                logger(f'freq_values_representation- {freq_values_representation}')
                logger(f'instance_freq_vec- {instance_freq_vec}')
...
if __name__ == '__main__':
    loop_candidates_convert_to_freq_vec(representation_option=DistMethod.fix_length_freq, representation_option_for_set=DistMethod.one_hot_vector, representation_option_for_nested=DistMethod.fix_length_freq)
