...
class DistanceStrFunctions:
    ...
    def q3(self) -> float:
        ...
        min_freq = min(
            self.str_distance_data.value_frequency.items(), key=lambda x: x[1])[1]
        my_print(f'q3; min frequency {min_freq}')
        val1_frequency = self.str_distance_data.value_frequency.get(
            self.str_distance_data.val1)
        val2_frequency = self.str_distance_data.value_frequency.get(
            self.str_distance_data.val2)
        my_print(f'q3; val1 frequency {val1_frequency}')
        my_print(f'q3; val2 frequency {val2_frequency}')
        max_freq = max(val1_frequency, val2_frequency)
        my_print(f'q3; max frequency {max_freq}')
...
class DistanceCatFunctions:
    ...
    def calc_num_distance_q13(self) -> float:
        my_print(
            f'numeric val1- {self.num_distance_data.val1} , val2- {self.num_distance_data.val2}')
