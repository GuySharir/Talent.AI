...
class LengthAttr:
    ...
    def nested_length(self, instance: list, nested_attr: str):
        ...
        for nested_obj in instance:
            for attr, val in nested_obj.items():
                val_type = locate(
                    self.nested_attr_types[nested_attr][attr].split("'")[1])
                if val_type == list:
    ...
    def length_check_per_attr(self):
        for i in range(0, len(self.df)):
            instance = {attr: self.df.iloc[i][inx]
        ...
        self.lis_lengths_per_attr_min_max = {key: {'min': min(val), 'max': max(
            val), 'median': statistics.median(val)} for key, val in self.lis_lengths_per_attr.items()}
        self.nested_lengths_per_attr_min_max = {key: {'min': min(val), 'max': max(
            val), 'median': statistics.median(val)} for key, val in self.nested_lengths_per_attr.items()}

        path = os.path.abspath(os.path.join(os.path.dirname(
            __file__), '..', f'dataTool\\runtimeObjectsInfo')).replace('/', '\\')
        length_path = os.path.abspath(
            os.path.join(path, 'number_of_elements.txt'))

        with open(length_path, 'w') as f:
            f.write(
                f'lists: {self.lis_lengths_per_attr_min_max}\nnested: {self.nested_lengths_per_attr_min_max}')
