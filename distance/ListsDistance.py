...
class ListsDistance:
    ...
    def intersection(self) -> float:
        union = list(set(self.list_distance_data.list1 +
                     self.list_distance_data.list2))
        intersection = list(set.intersection(
            set(self.list_distance_data.list1), set(self.list_distance_data.list2)))

        union_len = len(union)
        intersection_len = len(intersection)
        my_print(
            f'list1 {self.list_distance_data.list1}\nlist2 {self.list_distance_data.list2}')
        my_print(
    ...
    def descending_freq_order(self) -> list:
        sorted_freq = sorted(self.list_distance_data.value_frequency.items(
        ), key=lambda x: x[1], reverse=True)
        sorted_freq_values = [i[0] for i in sorted_freq]
        my_print(f'sorted freq in descending order- {sorted_freq_values}')

        return sorted_freq_values

    @staticmethod
    def order_list_by_freq(list_obj: list, sorted_freq_values: list) -> list:
        sorted_freq_in_list_obj = [
            x for x in sorted_freq_values if x in list_obj]
        result_list = sorted(
            list_obj, key=lambda ele: sorted_freq_in_list_obj.index(ele))
        return result_list
    ...
    def freq_list_all(self, ordered: bool = False) -> float:
        ...
        my_print(f'list length expected: {list_length_expected} ')
        ...
        my_print(f'lists after length preparation- 1: {list1}\n2: {list2} ')
        ...
        if ordered:
            ...
            list1 = self.order_list_by_freq(
                list_obj=list1, sorted_freq_values=sorted_freq_values)
            list2 = self.order_list_by_freq(
                list_obj=list2, sorted_freq_values=sorted_freq_values)
            my_print(f'ordered lists- 1: {list1}\n2: {list2} ')
            for i in range(list_length_expected):
                result = dist_obj.distance_per_type(val_type=str, val1=list1[i], val2=list2[i],
                                                    ...
                                                    instance_a={
                                                        self.list_distance_data.attribute: list1},
                                                    instance_b={self.list_distance_data.attribute: list2})
                list_dist_calc_result.append(result)
            my_print(f'list_dist_calc_result: {list_dist_calc_result}')
    ...
    def calc_dist(self) -> float:
        my_print(f'list attribute- {self.list_distance_data.attribute}')
        my_print(
            f'given lists- 1: {self.list_distance_data.list1}\n2: {self.list_distance_data.list2} ')
